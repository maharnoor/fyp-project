...
export function calculateRecommendations(answersWithQuestions) {
    const scores = { cs: 0, medical: 0, engineering: 0, business: 0, arts: 0 }

    answersWithQuestions.forEach(({ question, answer }) => {
        if (!question) return;
        
        const points = 3;
        if (answer === 0 && question.fieldTag) {
            scores[question.fieldTag] = (scores[question.fieldTag] || 0) + points;
        } else if (answer === 1) {
            scores['medical'] += points;
        } else if (answer === 2) {
            scores['business'] += points;
        } else if (answer === 3) {
            scores['arts'] += points;
        }
    })
