...
// Scoring weights for each question's answer -> field
export const QUIZ_SCORING = {
    // format: questionIndex -> { optionIndex -> { field: points } }
    0: { 0: { cs: 3, engineering: 2 }, 1: { medical: 3, biology: 2 }, 2: { business: 3 }, 3: { arts: 3 } },
    1: { 0: { cs: 3 }, 1: { engineering: 3 }, 2: { medical: 2, engineering: 1 }, 3: { arts: 3 } },
    2: { 0: { engineering: 3, cs: 2 }, 1: { medical: 3 }, 2: { business: 3 }, 3: { arts: 2 } },
    3: { 0: { cs: 3, engineering: 2 }, 1: { medical: 2 }, 2: { business: 3 }, 3: { arts: 3 } },
    4: { 0: { engineering: 3 }, 1: { medical: 3 }, 2: { cs: 3 }, 3: { business: 2, arts: 1 } },
}
...
export function calculateRecommendations(answers) {
    ...
    answers.forEach((answer, qIdx) => {
        const qScoring = QUIZ_SCORING[qIdx]
        if (qScoring && qScoring[answer]) {
            Object.entries(qScoring[answer]).forEach(([field, points]) => {
